...
//Rosenbrock Method
{
    ...
    try{
        if((int)x0.n_cols!=1)
            throw std::logic_error("ERROR: fmin(): x0 must be a column vector.");
    }
...
x_fval sci_arma::fmincon(const obj_fun &f, vec &x0, mat &A, mat &b, mat &Aeq, mat &beq, const non_linear_con &c, long n_non_lin)
{
    try{
        if((int)x0.n_cols!=1)
            throw std::logic_error("ERROR: fmincon(): x0 must be a column vector.");
